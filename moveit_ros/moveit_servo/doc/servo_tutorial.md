...
A standalone component is also offered as a `ServoComponentNode`. This allows you include starting a `ServoComponentNode` in your launch file, and interact with `Servo` only through ROS topics and services. See the [servo server demo](../launch/servo_component_demo.launch.py) as an example.
...
### Supporting Nodes
You will need to launch either a `ServoComponentNode` or the node containing your `Servo` C++ instance. In addition, `moveit_servo` needs a few other nodes to be running to work:
